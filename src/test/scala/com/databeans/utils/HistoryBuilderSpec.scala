...
class HistoryBuilderSpec extends AnyFlatSpec with Matchers with GivenWhenThen {
  ...
  "BuildResultSameAddress" should "Trait data within the DataFrame" in {
    ...
    val resultWindowSpecWithLedMovedInAndAddress = windowSpecWithLedMovedInAndAddress(resultUnionHistoryAndUpdateData)
    val resultBuildData = historyBuilder(resultWindowSpecWithLedMovedInAndAddress )
